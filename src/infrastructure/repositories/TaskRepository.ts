...
import type { ITaskRepository, TaskFilters } from "../../domain/repositories/ITaskRepository";
...
export class TaskRepository implements ITaskRepository {
  ...
  async getListByFilters(filters?: TaskFilters): Promise<Task[]> {
    ...
    const results = filters?.status ? await query.where(eq(tasks.status, filters.status)) : await query;
